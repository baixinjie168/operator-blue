...
- 控制在 1 到 3 句，保持原文语义。
- 如果文档完全没有函数描述，填写：`文档中未提供函数描述`

**严格对应校验**：
- `description` 必须严格对应当前函数，不要混入算子整体功能、计算公式、其他函数说明或当前函数原型中不存在的参数。

**两段式接口描述规范校验**：
- **第一段接口描述**：仅描述第一段接口职责，如"第一段接口：完成入参校验，获取计算所需的workspace大小以及包含算子计算流程的执行器。"，不应扩写算子总体功能说明。
- **第二段接口描述**：仅描述第二段接口职责，如"第二段接口：使用根据第一段接口获取的workspaceSize申请的workspace和返回的executor执行计算。"，不应复用算子功能说明。
...
**参数名一致性校验**：
- 函数签名与参数说明同时存在时，优先以参数说明表为准。
- keepdim/keepDim 大小写按文档参数说明表统一。
- workspaceAddr → workspace，dstType 等按文档原名。

**去重与完整性校验**：
- 不要把同一个形参拆成两个参数对象；应保留一个，并在 description 中表达其完整语义。
- 按文档参数说明表顺序补充遗漏的参数（如 relType），填写 name/role/type/is_optional/description/format。
...
**优先规则校验**：
- 当函数签名与参数说明表的类型不一致时，应按”参数说明优先于函数签名”规则提取。
...
- 如果参数描述缺失，填写：`文档中未提供该参数说明`

**原文准确性校验**：
- 严格按原始文档参数说明表提取，不要擅自修正或改写参数语义。
- 若确认原文有笔误，应先修正文档再同步更新 JSON。

**特殊参数描述统一校验**：
- **workspaceSize**：第一段接口的 workspaceSize 描述应为”返回需要在Device侧申请的workspace大小。”，第二段接口的 workspace 描述应为”在Device侧申请的workspace大小，由第一段接口XXXGetWorkspaceSize获取。”。
- **executor**：描述应为”返回包含了算子计算流程的执行器。”，不要与 workspaceSize 说明重复。
- **inplace 参数**：应补充双向语义，在 description 中补充”输入同时也是输出”或”原地更新”的表述。
...
**类型统一校验**：
- format 字段应为字符串（如 `”ND”`）或字符串数组（如 `[“NCL”, “NCHW”, “NCDHW”]`），不要写成 `[“ND”]` 再嵌套。
